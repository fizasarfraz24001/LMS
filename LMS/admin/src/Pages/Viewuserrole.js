...
const getCountry = async()=>{
    try{
        const response= await axios.get("http://localhost:5000/DisplayUserRole");
...
},
{
    name:"Delete",
    cell:(row)=>(
        <button className="btn btn-outline-primary" onClick={()=>deleteCategory(row.UID)}>Delete</button>
...
return country.UName.toLowerCase().match(search.toLowerCase());
...
const deleteCategory=(UID)=>{
    if(window.confirm("Are you sure you want to delete this record")){
     ...
     axios.delete(`http://localhost:5000/DeleteUserRole/${UID}`)
      .then(() =>{
...
<p id="h11" class="d-flex justify-content-center">View UserRole</p>
</div>        </div>
